# PaperBrain â€” project plan & delivery status
...
- [x] **5-provider fallback chain** â€” ordered priority: OpenRouter â†’ Groq â†’ Gemini â†’ HuggingFace â†’ OpenAI
...
- [x] **Auto-loaded on startup** if existing index found â€” no re-upload needed after restart
...
- [x] **Model selector** â€” fetches live `/models` from backend, falls back to static list; shows provider badge
- [x] **Model id wired to `/ask`** â€” selected model passed through `useChat` â†’ `api.askQuestion(message, model, includeSources)`
- [x] **Include-sources toggle** â€” collapsible source citations on assistant messages
- [x] **Streaming toggle** â€” switch between SSE streaming and standard JSON mode
- [x] **Export chat** â€” download history as `.txt` (also `Ctrl/Cmd+Shift+E`)
- [x] **Health indicator** â€” green/red/yellow dot in header polling `/health` every 15s
...
- [x] **localStorage** â€” persists selected model, include-sources toggle, streaming toggle, banner dismissal
- [x] **IndexedDB** â€” chat history saved per-PDF (keyed by filename); auto-restored on re-upload of same PDF
- [x] **Saved sessions panel** â€” list / restore / delete previous chat sessions from IndexedDB
- [x] **Device-local data banner** â€” dismissible amber notice explaining data stays in this browser
- [x] All storage operations wrapped in try/catch â€” degrades gracefully in private mode / quota limits
...
IndexedDB + `localStorage` gives each visitor "their own" chat history, preferences, and cached metadata â€” entirely on that one browser. No server-side user accounts, cookies, or databases involved.
...
| **Auth** | Session/JWT/cookies | Tie every API request to a verified user/tenant. Even magic-link or OAuth counts â€” the point is server-enforced identity. |
...
Browser storage (IndexedDB + localStorage) can still serve as a **UX cache** on top of server state â€” not the source of truth.
...
Why This Approach Works Well
RAG works well because it keeps answers grounded in real data. The model is not guessing â€“ it's reading from your document.
