# Docker â€” lean VPS backend playbook (Coolify / Hetzner)

Copy this checklist into new Python (or similar) backend repos you deploy with **Dockerfile + Coolify**. It maps common â€œsmall imageâ€ practices to **real** backend constraints (Python wheels, system libs, secrets).

## This Project (PaperBrain) â€” applied profile
...
## 1. Seven practices â†’ Python / FastAPI reality

| # | Practice | Python / VPS notes |
|---|----------|-------------------|
| 1 | **Small base image** | Prefer `python:X.Y-slim` (Debian glibc). **Alpine** is smaller but many PyPI wheels lack musl builds â€” use Alpine only if you verify every dependency. Avoid unversioned `python:latest`. |
...
| 6 | **`.dockerignore`** | Exclude `.git`, `frontend/`, `.env*`, `__pycache__`, tests, docs you do not need in the image â€” smaller context, faster builds, fewer accidental secret leaks. |
...
**Reality check:** viral â€œ1.5 GB â†’ 50 MBâ€ posts are often **Node** (dropping `node_modules` + build tools). Python API images are dominated by **base + PyPI deps + optional native libs** (e.g. `ffmpeg`). Aim for **lean + correct**, not arbitrary MB targets.
...
# Runtime OS deps only â€” one layer, lists cleaned
RUN apt-get update \
...
**Coolify:** set `PORT=3000`, map host port â†’ `3000`, Traefik `loadbalancer.server.port=3000` (see [COOLIFY_PUBLIC_BACKEND_GUIDE.md](./COOLIFY_PUBLIC_BACKEND_GUIDE.md)).

**Optional â€” pin base digest** (reproducible builds; refresh when you intentionally upgrade):
...
- **Secrets:** set API keys only in Coolify env â€” never commit `.env`. If a deploy log ever prints runtime env with keys, **rotate** those keys at the provider.
- **Healthcheck:** Coolify UI healthcheck can disagree with app health; validate with `curl https://<your-domain>/api/health`.
- **TLS:** verify issuer with `openssl s_client` (see [COOLIFY_PUBLIC_BACKEND_GUIDE.md](./COOLIFY_PUBLIC_BACKEND_GUIDE.md) Â§6).
...
**Aggressive** (removes **all** unused images, not only dangling â€” next Coolify build may re-pull helper/base layers; Coolify rollback images may disappear):
...
## 7. Blog-to-audioâ€“specific note
...
This section answers â€œ**is it safe to commit and redeploy?**â€, â€œ**will it get slower?**â€, and â€œ**is this the optimal setup?**â€ â€” with nuance per **project shape**.

### 8.1 Safe to commit and redeploy (typical Coolify flow)

For this repoâ€™s pattern (**FastAPI + uvicorn + `PORT` env + slim + optional `ffmpeg` + non-root user**):

- **Yes â€” safe** to push, let Coolify rebuild, and roll out, as long as **Coolify env** (`PORT`, API keys, `CORS_ORIGINS`, etc.) is unchanged and **Traefik/Caddy labels** still point at the same container port (usually **3000**).
...
- **What can change after `USER`:** only **paths the app must write to** must be owned or writable by that user (here: `/app` and `audio_files/`). Read-only system paths and site-packages remain readable by default â€” same as before.

### 8.2 Performance: will the VPS feel â€œslowerâ€?
...
| Heavy ML / torch stacks | **Large RAM + slow cold start** â€” that dominates far more than Docker â€œoptimizationsâ€. |

**Bottom line:** For blog-to-audio, **TTS providers and text length** dominate latency â€” not Docker micro-optimizations.

### 8.3 â€œOptimal bestâ€ depends on architecture â€” decision matrix

There is **no single global best Dockerfile**. Pick patterns from **constraints**:

| Your project | Lean Docker emphasis |
|--------------|----------------------|
| **Small FastAPI API** (few files, pure wheels, one process) | `slim` + `.dockerignore` + non-root + `HEALTHCHECK` + copy `requirements.txt` first â€” **enough**. Multi-stage optional until you add native compile deps. |
...
| **Monorepo** (React + API in one repo) | API image: `.dockerignore` must exclude **`frontend/`**, `node_modules/`, tests â€” **never** ship the SPA inside the Python image unless you intend to. |
| **Backend in `backend/` subfolder** | Coolify **Base directory** = `backend`; Dockerfile path relative to that folder; keep `.dockerignore` at repo root or duplicate rules â€” consistency matters more than which file you pick. |
...
| **Database in same container** | Generally **avoid** for production; run Postgres/MySQL as **Coolify managed service** or external â€” simpler backups and upgrades. |
| **GPU** | Needs **NVIDIA runtime** + different base images â€” outside this playbookâ€™s default `slim` path. |
| **Many replicas behind a LB** | Each replica is an **immutable** container; **shared disk** must be NFS/object storage, not local `audio_files` only â€” architecture change, not Dockerfile-only. |
...
| **Healthcheck red but app works** | Coolify probe path/port/text mismatch â€” fix probe or rely on Dockerfile `HEALTHCHECK` + `curl` from outside. |
| **TLS / privacy errors** | Usually **ACME / DNS / labels**, not Dockerfile â€” see subdomain doc. |
| **Permission denied writing files** | Almost always **non-root + volume mount ownership** â€” fix mount UID/GID or `chown` in entrypoint (only if you must; prefer volumes created with correct ownership). |
| **Logs printed secrets** | **Rotate keys** â€” Dockerfile cannot fix logging mistakes in the platform. |

### 8.6 Copy-paste reassurance (blog-to-audioâ€“style stack)

**Dockerfile / VPS:** Committing and redeploying a **`python:3.12-slim`** image with **minimal `apt`** (here **`ffmpeg`** where audio processing needs it), **`.dockerignore`**, **`pip --no-cache-dir`**, and a **non-root user** is a **solid, pragmatic** production baseline on Coolify. It is **not** inherently slower than root for normal HTTP workloads. **â€œOptimal bestâ€** always depends on needs: add **multi-stage** when compile tooling or extra build steps would otherwise bloat the runtime image; add **digest-pinned `FROM`** when you need stricter reproducibility than a rolling `slim` tag.
