# Coolify + custom domain + HTTPS â€” public template (VPS backend)
...
## This Project (PaperBrain) â€” quick mapping

- **Backend path:** `backend/`
- **Framework:** FastAPI + Uvicorn
- **Container/app port:** `3000`
- **Public API domain used here:** `https://rag-pdf-backend.yourdomain.com`
- **Frontend origin used here:** `https://your-frontend.vercel.app`
- **Key health/runtime routes:** `/health`, `/status`, `/runtime-summary`, `/ask`, `/ask/stream`, `/upload`
- **Frontend env to point at backend:** `VITE_API_BASE_URL=https://rag-pdf-backend.yourdomain.com`
- **Backend CORS env example:** `CORS_ORIGINS=https://your-frontend.vercel.app`
...
| `YOUR_API_SUBDOMAIN` | DNS label only (e.g. `api` â†’ `api.example.com`) |
| `YOUR_API_FQDN` | Full host (e.g. `api.example.com`) |
| `YOUR_COOLIFY_SSLIP_HOST` | Coolifyâ€™s auto hostname, usually `*.YOUR_VPS_PUBLIC_IP.sslip.io` (copy from Coolify **Domains** for the resource) |
...
| `YOUR_HOST_PORT` | Optional host map in Coolify (e.g. `5005`) â€” `YOUR_HOST_PORT:YOUR_CONTAINER_PORT` |
...
## 3. Coolify â€” new application

1. **New resource â†’ Application** â†’ connect Git, pick branch.
...
Under the application **General â†’ Domains**, add:
...
1. **sslip.io** host (Coolify default) â€” keeps a working URL even before DNS is ready.
2. **Your real FQDN** â€” Letâ€™s Encrypt certificate via `certresolver=letsencrypt`.
...
- `` `YOUR_COOLIFY_SSLIP_HOST` `` â€” exact string from Coolify (no `https://`).
- `` `YOUR_API_FQDN` `` â€” production API host only.
- **All** occurrences of `REPLACE_SSLIP_TOKEN` with the same token you use in router names (often derived from the sslip hostname without dots, or use Coolifyâ€™s generated fragment).
...
**Important:** the **Host(`...`)** rule for sslip must match Coolifyâ€™s string **exactly** (typos â†’ wrong cert or 404).
...
If the browser shows **certificate errors** while sslip works, the problem is almost always **DNS, labels, or ACME** â€” not application code.
...
See [DOCKER_VPS_BACKEND_PLAYBOOK.md](./DOCKER_VPS_BACKEND_PLAYBOOK.md) Â§6 for **`docker â€¦ prune`** commands and warnings about **`docker image prune -a`**.
