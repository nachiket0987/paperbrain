 ...
 * - Local (via Vite proxy):    /api   â†’ proxy strips prefix to backend
 ...
 * Empty `VITE_API_BASE_URL` in dev â†’ http://localhost:8000
 * Empty in production build â†’ console error + localhost fallback (set Vercel env!)
 */
export function resolveApiBaseUrl(): string {
  ...
  if (trimmed) {
    if (trimmed.startsWith("/")) {
      ...
      if (rel === "/") {
        ...
        console.warn(
          "[paperbrain] VITE_API_BASE_URL=/ is not a valid API base; using http://localhost:8000.",
        );
        return "http://localhost:8000";
      }
      return rel;
  ...
  if (import.meta.env.PROD) {
    console.error(
      "[paperbrain] VITE_API_BASE_URL is missing. Add it in Vercel â†’ Settings â†’ Environment Variables (your Coolify API URL).",
    );
  }
 ...
 * - Absolute API base: https://api.example.com â†’ https://api.example.com/api/oversight
 * - Relative dev base: /api â†’ /api/oversight (Vite proxy forwards to backend)
 */
